// export const culturalHighlights = [
//   {
//     id: "1",
//     title: "Sarhul Festival",
//     description: "Ancient spring festival celebrating nature and tribal unity with traditional dances and songs.",
//     date: "March - April",
//     participants: "50+ Communities",
//     //   image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=500&h=300&fit=crop",
//     image: "https://media.assettype.com/outlooktraveller%2Fimport%2Foutlooktraveller%2Fpublic%2Fuploads%2Farticles%2Fexplore%2FSarhul.jpg?w=1200&ar=40%3A21&auto=format%2Ccompress&ogImage=true&mode=crop&enlarge=true&overlay=false&overlay_position=bottom&overlay_width=100",
//     ytKey: "shorts/ef5e4RgD5i4"
//   },
//   {
//     id: "2",
//     title: "Tribal Art & Crafts", // Tribal Art & Crafts Intricate handwoven textiles, bamboo crafts, and traditional pottery by skilled artisans
//     description: "Intricate handwoven textiles, bamboo crafts, and traditional pottery by skilled artisans.",
//     date: "Year Round",
//     participants: "200+ Artisans",
//     image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&h=300&fit=crop",
//     ytKey: "shorts/TdboRQvRtII"
//   },
//   {
//     id: "3",
//     title: "Folk Music Traditions", // Folk Music Traditions Centuries-old musical heritage featuring traditional instruments and storytelling ballads.
//     description: "Centuries-old musical heritage featuring traditional instruments and storytelling ballads.",
//     date: "Cultural Events",
//     participants: "30+ Music Groups",
//     image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&h=300&fit=crop",
//     ytKey: "shorts/QSFqRN8os8g"
//   }
// ];





// const culturalHighlights = [
//     {
//       title: "Sarhul Festival",
//       description: "Ancient spring festival celebrating nature and tribal unity with traditional dances and songs.",
//       date: "March - April",
//       participants: "50+ Communities",
//       image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=500&h=300&fit=crop",
//       duration: "3 Days", // ****
//       location: "Statewide"
//     },
//     {
//       title: "Karma Festival",
//       description: "Harvest festival featuring the sacred Karma tree worship with folk dances and music.",
//       date: "August - September",
//       participants: "40+ Communities",
//       image: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500&h=300&fit=crop",
//       duration: "2 Days",
//       location: "Rural Areas"
//     },
//     {
//       title: "Tribal Art & Crafts",
//       description: "Intricate handwoven textiles, bamboo crafts, and traditional pottery by skilled artisans.",
//       date: "Year Round",
//       participants: "200+ Artisans",
//       image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&h=300&fit=crop",
//       duration: "Ongoing",
//       location: "Craft Villages"
//     },
//     {
//       title: "Jani Shikar",
//       description: "Traditional hunting festival marking the end of monsoon with community celebrations.",
//       date: "September - October",
//       participants: "25+ Villages",
//       image: "https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=500&h=300&fit=crop",
//       duration: "1 Day",
//       location: "Forest Areas"
//     },
//     {
//       title: "Tusu Festival",
//       description: "Winter harvest celebration with traditional songs, dances and colorful decorations.",
//       date: "December - January",
//       participants: "60+ Communities",
//       image: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500&h=300&fit=crop",
//       duration: "15 Days",
//       location: "Villages"
//     },
//     {
//       title: "Folk Music Traditions",
//       description: "Centuries-old musical heritage featuring traditional instruments and storytelling ballads.",
//       date: "Cultural Events",
//       participants: "30+ Music Groups",
//       image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&h=300&fit=crop",
//       duration: "Varies",
//       location: "Cultural Centers"
//     }
//   ];




export const culturalHighlights = [
  {
    id: "1",
    title: "Sarhul Festival",
    description: "Ancient spring festival celebrating nature and tribal unity with traditional dances and songs.",
    date: "March - April",
    participants: "50+ Communities",
    image: "https://media.assettype.com/outlooktraveller%2Fimport%2Foutlooktraveller%2Fpublic%2Fuploads%2Farticles%2Fexplore%2FSarhul.jpg?w=1200&ar=40%3A21&auto=format%2Ccompress&ogImage=true&mode=crop&enlarge=true&overlay=false&overlay_position=bottom&overlay_width=100",
    duration: "3 Days",
    location: "Statewide",
    ytKey: "shorts/ef5e4RgD5i4"
  },
  {
    id: "2",
    title: "Karma Festival",
    description: "Harvest festival featuring the sacred Karma tree worship with folk dances and music.",
    date: "August - September",
    participants: "40+ Communities",
    image: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500&h=300&fit=crop",
    duration: "2 Days",
    location: "Rural Areas",
    ytKey: "shorts/defaultKey"
  },
  {
    id: "3",
    title: "Tribal Art & Crafts",
    description: "Intricate handwoven textiles, bamboo crafts, and traditional pottery by skilled artisans.",
    date: "Year Round",
    participants: "200+ Artisans",
    image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&h=300&fit=crop",
    duration: "Ongoing",
    location: "Craft Villages",
    ytKey: "shorts/TdboRQvRtII"
  },
  {
    id: "4",
    title: "Jani Shikar",
    description: "Traditional hunting festival marking the end of monsoon with community celebrations.",
    date: "September - October",
    participants: "25+ Villages",
    image: "https://media.assettype.com/outlooktraveller%2Fimport%2Foutlooktraveller%2Fpublic%2Fuploads%2Farticles%2Fexplore%2FSarhul.jpg?w=1200&ar=40%3A21&auto=format%2Ccompress&ogImage=true&mode=crop&enlarge=true&overlay=false&overlay_position=bottom&overlay_width=100",
    duration: "1 Day",
    location: "Forest Areas",
    ytKey: "shorts/defaultKey"
  },
  {
    id: "5",
    title: "Tusu Festival",
    description: "Winter harvest celebration with traditional songs, dances and colorful decorations.",
    date: "December - January",
    participants: "60+ Communities",
    image: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500&h=300&fit=crop",
    duration: "15 Days",
    location: "Villages",
    ytKey: "shorts/defaultKey"
  },
  {
    id: "6",
    title: "Folk Music Traditions",
    description: "Centuries-old musical heritage featuring traditional instruments and storytelling ballads.",
    date: "Cultural Events",
    participants: "30+ Music Groups",
    image: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=500&h=300&fit=crop",
    duration: "Varies",
    location: "Cultural Centers",
    ytKey: "shorts/QSFqRN8os8g"
  }
];



// const traditions = [
//     { name: "Santhal", population: "600,000+", specialty: "Traditional Dance & Music" },
//     { name: "Munda", population: "450,000+", specialty: "Agricultural Festivals" },
//     { name: "Oraon", population: "350,000+", specialty: "Forest Conservation" },
//     { name: "Ho", population: "200,000+", specialty: "Metalwork & Crafts" }
//   ];

export const tribalCommunities = [
  {
    name: "Santhal",
    population: "600,000+",
    specialty: "Traditional Dance & Music",
    region: "Eastern Jharkhand",
    language: "Santhali"
  },
  {
    name: "Munda",
    population: "450,000+",
    specialty: "Agricultural Festivals",
    region: "Central Jharkhand",
    language: "Mundari"
  },
  {
    name: "Oraon",
    population: "350,000+",
    specialty: "Forest Conservation",
    region: "Western Jharkhand",
    language: "Kurukh"
  },
  {
    name: "Ho",
    population: "200,000+",
    specialty: "Metalwork & Crafts",
    region: "Southern Jharkhand",
    language: "Ho"
  },
  {
    name: "Kharia",
    population: "150,000+",
    specialty: "Bamboo Crafts",
    region: "Northern Jharkhand",
    language: "Kharia"
  },
  {
    name: "Kharwar",
    population: "120,000+",
    specialty: "Traditional Weaving",
    region: "Scattered",
    language: "Kharwar"
  }
];


export const touristSpotsMap = [
  {
    id: 1,
    name: "Hundru Falls",
    category: "nature",
    lat: 23.4241,
    lng: 85.5867,
    description: "300 feet waterfall near Ranchi",
    rating: 4.5,
    image: "🏞️"
  },
  {
    id: 2,
    name: "Betla National Park",
    category: "wildlife",
    lat: 23.8667,
    lng: 84.1833,
    description: "Home to tigers, elephants and rich biodiversity",
    rating: 4.7,
    image: "🐅"
  },
  {
    id: 3,
    name: "Sun Temple Bundu",
    category: "cultural",
    lat: 23.1667,
    lng: 85.5833,
    description: "Ancient temple dedicated to Sun God",
    rating: 4.2,
    image: "🏛️"
  },
  {
    id: 4,
    name: "Jagannath Temple Ranchi",
    category: "cultural",
    lat: 23.3441,
    lng: 85.3096,
    description: "Replica of Puri Jagannath Temple",
    rating: 4.4,
    image: "🛕"
  },
  {
    id: 5,
    name: "Parasnath Hills",
    category: "nature",
    lat: 23.9633,
    lng: 86.1633,
    description: "Highest peak in Jharkhand, sacred Jain pilgrimage",
    rating: 4.6,
    image: "⛰️"
  },
  {
    id: 6,
    name: "Dassam Falls",
    category: "nature",
    lat: 23.5000,
    lng: 85.5167,
    description: "39.62 meter high waterfall on Kanchi River",
    rating: 4.3,
    image: "🏞️"
  },
  {
    id: 7,
    name: "Palamu Tiger Reserve",
    category: "wildlife",
    lat: 23.8667,
    lng: 84.1833,
    description: "First Tiger Reserve of India established in 1974",
    rating: 4.6,
    image: "🐅"
  },
  {
    id: 8,
    name: "Deori Temple",
    category: "cultural",
    lat: 23.5000,
    lng: 84.8833,
    description: "Ancient Shiva temple in Tamar village",
    rating: 4.1,
    image: "🛕"
  },

];



// wind life home page -->

export const sanctuaries = {
  betla: {
    name: "Betla National Park",
    area: "979 sq km",
    established: "1986",
    animals: [
      "Royal Bengal Tiger",
      "Asian Elephant",
      "Wild Boar",
      "Sloth Bear",
    ],
    birds: [
      "Peacock",
      "Red Junglefowl",
      "White-rumped Vulture",
      "Crested Serpent Eagle",
    ],
    activities: [
      "Tiger Safari",
      "Elephant Tracking",
      "Bird Watching",
      "Nature Photography",
    ],
    bestTime: "October - March",
    image:
      "https://india-tours.com/wildlife/images/wildlife/national-parks/betla-national-park.jpg"
  },
  palamu: {
    name: "Palamu Tiger Reserve",
    area: "1026 sq km",
    established: "1974",
    animals: ["Tiger", "Leopard", "Wild Dog", "Sambhar Deer"],
    birds: [
      "Great Hornbill",
      "Pied Hornbill",
      "White-backed Vulture",
      "Painted Stork",
    ],
    activities: [
      "Wildlife Safari",
      "Fort Exploration",
      "Trekking",
      "Research Tours",
    ],
    bestTime: "November - April",
    image:
      "https://images.unsplash.com/photo-1578662996442-48f60103fc96?w=500&h=300&fit=crop",
  },
};


export const conservationStats = [
  {
    icon: "🎯",
    number: "150+",
    label: "Species Protected",
    color: "text-green-600",
  },
  {
    icon: "🌲",
    number: "2000+",
    label: "Sq Km Forest",
    color: "text-green-800",
  },
  {
    icon: "🦅",
    number: "300+",
    label: "Bird Species",
    color: "text-blue-600",
  },
  {
    icon: "🐟",
    number: "50+",
    label: "Fish Varieties",
    color: "text-teal-600",
  },
];

export const stories = [
  {
    id: 1,
    name: "name surname",
    role: "Traditional Craft Artisan",
    village: "Khunti Village",
    story: "Tourism has helped preserve our ancient bamboo weaving techniques. Young people are now learning these skills and earning livelihoods while keeping our culture alive.",
    image: "https://india-tours.com/wildlife/images/wildlife/national-parks/betla-national-park.jpg",
    impact: "Trained 25+ youth in traditional crafts",
    rating: 5
  },
  {
    id: 2,
    name: "name surname",
    role: "Community Homestay Host",
    village: "Betla Region",
    story: "Hosting tourists in our homestay has not only improved our family's income but also created cultural exchange. Visitors learn about our traditions while we learn about the world.",
    image: "https://india-tours.com/wildlife/images/wildlife/national-parks/betla-national-park.jpg",
    impact: "Hosted 200+ families from 15 countries",
    rating: 5
  },
  {
    id: 3,
    name: "name surname",
    role: "Eco-Guide & Forest Protector",
    village: "Palamu Forest Area",
    story: "As an eco-guide, I help tourists understand our forest ecosystem while ensuring wildlife protection. This work has made conservation profitable for our community.",
    image: "https://india-tours.com/wildlife/images/wildlife/national-parks/betla-national-park.jpg",
    impact: "Guided 500+ wildlife tours safely",
    rating: 5
  },
];

export const communityImpacts = [
  { number: "1,200+", label: "Families Benefited" },
  { number: "₹2.5 Cr", label: "Community Income Generated" },
  { number: "85%", label: "Youth Retention Rate" },
  { number: "50+", label: "Villages Participating" }
];

// detination section and page

export const destinationsPageData = [
  {
    id: "1",
    name: "Betla National Park",
    location: "Latehar District",
    duration: "2-3 Days",
    rating: 4.8,
    reviews: 245,
    image: "https://images.unsplash.com/photo-1516026672322-bc52d61a55d5?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2048&q=80",
    description: "Home to tigers, elephants, and diverse wildlife in pristine forest landscapes.",
    categories: ["wildlife", "nature"],
    tags: ["Tiger Safari", "Elephant Spotting", "Bird Watching"],
    price: "₹2,500"
  },
  {
    id: "2",
    name: "Dassam Falls",
    location: "Taimara Village",
    duration: "1 Day",
    rating: 4.6,
    reviews: 189,
    image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2048&q=80",
    description: "Spectacular 144-foot waterfall surrounded by lush green hills and tribal villages.",
    categories: ["nature", "adventure"],
    tags: ["Waterfall Trek", "Photography", "Tribal Culture"],
    price: "₹800"
  },
  {
    id: "3",
    name: "Hundru Falls",
    location: "Ranchi",
    duration: "1 Day",
    rating: 4.7,
    reviews: 312,
    image: "https://images.unsplash.com/photo-1544551763-46a013bb70d5?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2048&q=80",
    description: "Majestic 320-foot waterfall cascading from great height, perfect for adventure enthusiasts.",
    categories: ["adventure", "nature"],
    tags: ["Rock Climbing", "Rappelling", "Nature Walk"],
    price: "₹1,200"
  },
  {
    id: "4",
    name: "Parasnath Hills",
    location: "Giridih District",
    duration: "2 Days",
    rating: 4.5,
    reviews: 156,
    image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2048&q=80",
    description: "Sacred Jain pilgrimage site with ancient temples and breathtaking hill views.",
    categories: ["heritage", "nature"],
    tags: ["Temple Visit", "Pilgrimage", "Trekking"],
    price: "₹1,800"
  },
  {
    id: "5",
    name: "Tribal Cultural Villages",
    location: "Various Districts",
    duration: "3-4 Days",
    rating: 4.9,
    reviews: 98,
    image: "https://images.unsplash.com/photo-1578662996442-48f60103fc96?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2048&q=80",
    description: "Experience authentic tribal culture, traditional dances, and local crafts.",
    categories: ["culture"],
    tags: ["Cultural Immersion", "Traditional Dance", "Handicrafts"],
    price: "₹3,500"
  },
  {
    id: "6",
    name: "Netarhat Hills",
    location: "Latehar District",
    duration: "2 Days",
    rating: 4.4,
    reviews: 203,
    image: "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=2048&q=80",
    description: "Queen of Chotanagpur known for stunning sunrise and sunset views.",
    categories: ["nature", "adventure"],
    tags: ["Sunrise View", "Hill Station", "Nature Photography"],
    price: "₹2,200"
  }
];



export const destinationsSectionData = [
  {
    id: 1,
    name: "Betla National Park",
    location: "Latehar District",
    description:
      "Home to tigers, elephants, and diverse wildlife in pristine forest landscapes.",
    duration: "2-3 Days",
    rating: 4.8,
    reviews: 245,
    category: "Wildlife",
    image:
      "https://images.unsplash.com/photo-1547036967-23d11aacaee0?w=500&h=300&fit=crop",
    highlights: ["Tiger Safari", "Elephant Spotting", "Bird Watching"],
  },
  {
    id: 2,
    name: "Dassam Falls",
    location: "Taimara Village",
    description:
      "Spectacular 144-foot waterfall surrounded by lush green hills and tribal villages.",
    duration: "1 Day",
    rating: 4.6,
    reviews: 189,
    category: "Nature",
    image:
      "https://images.unsplash.com/photo-1508193638397-1c4234db14d8?w=500&h=300&fit=crop",
    highlights: ["Waterfall Trek", "Photography", "Tribal Culture"],
  },
  {
    id: 3,
    name: "Hundru Falls",
    location: "Ranchi",
    description:
      "Majestic waterfall cascading from 320 feet, perfect for adventure enthusiasts.",
    duration: "1 Day",
    rating: 4.7,
    reviews: 312,
    category: "Adventure",
    image:
      "https://images.unsplash.com/photo-1515187029135-18ee286d815b?w=500&h=300&fit=crop",
    highlights: ["Rock Climbing", "Rappelling", "Nature Walk"],
  },
  {
    id: 4,
    name: "Tribal Heritage Museum",
    location: "Ranchi",
    description:
      "Explore rich tribal heritage through artifacts, art, and cultural exhibitions.",
    duration: "Half Day",
    rating: 4.5,
    reviews: 156,
    category: "Culture",
    image:
      "https://images.unsplash.com/photo-1595789672028-99ed8693c24b?w=500&h=300&fit=crop",
    highlights: ["Tribal Art", "Cultural Shows", "Handicrafts"],
  },
  {
    id: 5,
    name: "Palamu Tiger Reserve",
    location: "Latehar District",
    description:
      "India's first tiger reserve with diverse flora, fauna, and ancient fort ruins.",
    duration: "3-4 Days",
    rating: 4.9,
    reviews: 198,
    category: "Wildlife",
    image:
      "https://images.unsplash.com/photo-1602524206755-7fd2f7370031?w=500&h=300&fit=crop",
    highlights: ["Tiger Tracking", "Fort Exploration", "Bird Sanctuary"],
  },
  {
    id: 6,
    name: "Tagore Hill",
    location: "Ranchi",
    description:
      "Historic hill where Rabindranath Tagore wrote many of his famous works.",
    duration: "Half Day",
    rating: 4.4,
    reviews: 267,
    category: "Heritage",
    image:
      "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=500&h=300&fit=crop",
    highlights: ["Literary Heritage", "Sunrise Views", "Peaceful Gardens"],
  },
];

